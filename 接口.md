...
| Vector\<Vector\<TerrainType\>\> | terrainMap | 所有的地形 |
| Vector\<Vector\<LandformType\>\> | landformMap | 所有的地貌 |
| Vector\<Vector\<ResourceType\>\> | resourceMap | 所有的资源 |
...
| LandSquare getSquare(Position position) | 返回对应position的单元块            |
| boolean isLegalPosition(int x, int y)           |判断位置是否合法             |
| boolean isLegalPosition(Position position)      |判断位置是否合法             |
| void initTemperature()                            |初始化温度分布          |    
| void initMoisture()                               | 初始化降水分布         | 
| void initTerrain_PLAIN_HILL()                     |生成普通地形          |  
| void initLandform()                               |随机生成地貌          |  
| void initResource()                               | （随机）生成资源         |   
| void clearForTerr(int x, int y, TerrainType terr) | 随机生成山河湖, 辅助方法，清理其他地形地貌资源         |  
| void applyRidge(int serial, int x, int y)         | 以某单元格为基点放入索引为serial的山系         |   
| void applyRiver(int serial, int x, int y)         | 以某单元格为基点放入索引为serial的河流        |  
| initTerrain_RIDGE_RIVER_LAKE()                     
